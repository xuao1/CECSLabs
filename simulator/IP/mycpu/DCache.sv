...
/* -------------- 8 read data control: choose data from mem or return buffer -------------- */
    ...
    assign rdata_ret    = uncached == 1 ? {96'b0, ret_buf[BIT_NUM-1:BIT_NUM-32]} : ret_buf >> {addr_pipe[BYTE_OFFSET_WIDTH-1:2], 5'b0};
    ...
    always_ff @(posedge clk) begin
        if(!rstn) begin
            ...
            // Lab6 TODO：generate wbuf for uncache here
                ...
                wbuf <= {wbuf[BIT_NUM-33:0], wdata_pipe};
            end
...
/* -------------- 12 miss address buffer: store the address of the cache line to be write -------------- */
    always_ff @(posedge clk) begin
        if(!rstn) begin
            ...
            if(uncached == 1) begin
                maddr_buf <= {addr_pipe[31:BYTE_OFFSET_WIDTH], {BYTE_OFFSET_WIDTH{1'b0}}};
            end
            else begin
                maddr_buf <= {
...
/* -------------- 14 memory settings -------------- */
    ...
    assign d_raddr  = uncached == 1 ? addr_pipe : {addr_pipe[31:BYTE_OFFSET_WIDTH], {BYTE_OFFSET_WIDTH{1'b0}}};
    assign d_rsize  = uncached == 1 ? rsize_pipe : 3'h2;
    assign d_rlen   = uncached == 1 ? 0 : WORD_NUM - 1;
    assign d_waddr  = maddr_buf;
    assign d_wsize  = 3'h2;
    assign d_wlen   = uncached == 1 ? 0 : WORD_NUM - 1;
    assign d_wdata  = wbuf[31:0];
    assign d_wstrb  = uncached == 1 ? wstrb_pipe : 4'b1111;
...
/* -------------- 16 write fsm: for write back-------------- */
    ...
    logic [WORD_OFFSET_WIDTH:0] write_num =  uncached == 1 ? 0 : WORD_NUM - 1;
