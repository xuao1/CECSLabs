...
bool isa_difftest_checkregs(CPU_state *ref_r, vaddr_t pc) {
  ...
  for (int i = 0; i < 32; i++){
    if (ref_r->gpr[i] != sim_cpu.gpr[i]) {
      printf("gpr[%d] is different before executing instruction at pc = " FMT_WORD ", gpr[%d] right = " FMT_WORD ", wrong = " FMT_WORD "\n",
        i, pc, i, ref_r->gpr[i], sim_cpu.gpr[i]); 
      return false;
    }
  }
  // check pc
  ...
  if (pc != sim_cpu.pc) {
    printf("pc is different before executing instruction at pc = " FMT_WORD ", pc right = " FMT_WORD ", wrong = " FMT_WORD "\n",
      pc, pc, sim_cpu.pc); 
    return false;
  }
