...
#define TRACE_QUEUE_SIZE 16

struct {
  inst_log logs[TRACE_QUEUE_SIZE];
  int front, rear;
  bool full;
} trace_queue = {{0}, 0, 0, false};

void enqueue_trace(word_t pc, word_t inst) {
  if(trace_queue.full) {
    trace_queue.front = (trace_queue.front + 1) % TRACE_QUEUE_SIZE;
  }
  trace_queue.logs[trace_queue.rear].pc = pc;
  trace_queue.logs[trace_queue.rear].inst = inst;
  trace_queue.rear = (trace_queue.rear + 1) % TRACE_QUEUE_SIZE;
  trace_queue.full = (trace_queue.rear == trace_queue.front);
}

// 清空队列
void empty_trace() {
  trace_queue.front = trace_queue.rear = 0;
  trace_queue.full = false;
}

void print_trace(){
  int i = trace_queue.front;
  // 如果队列满，打印整个队列
  if(trace_queue.full) {
    do{
      char disasm_buf[100];
      disassemble(disasm_buf, sizeof(disasm_buf), trace_queue.logs[i].pc, (uint8_t*)&trace_queue.logs[i].inst, 4);
      printf("pc = " FMT_WORD ", inst = " FMT_WORD ", code = %s\n", trace_queue.logs[i].pc, trace_queue.logs[i].inst, disasm_buf);
      i = (i + 1) % TRACE_QUEUE_SIZE;
    }while(i != trace_queue.rear);
  }
  // 否则，打印队列中的内容
  else{
    while(i != trace_queue.rear){
      ...
      disassemble(disasm_buf, sizeof(disasm_buf), trace_queue.logs[i].pc, (uint8_t*)&trace_queue.logs[i].inst, 4);
      printf("pc = " FMT_WORD ", inst = " FMT_WORD ", code = %s\n", trace_queue.logs[i].pc, trace_queue.logs[i].inst, disasm_buf);
      // FMT_WORD 已经在 common.h 中定义，为 "0x%08x"
      i = (i + 1) % TRACE_QUEUE_SIZE;
    }
  }
}
...
void single_cycle() {
  ...
  // 将 dut->clk 的值置为 1，执行一次 dut->eval()，产生时钟的上升沿；
  dut->clk = 1;
  dut->eval();
  ...
  if(dut->commit_wb == 1) {
    enqueue_trace(dut->pc_cur, dut->inst);
    set_state();
  }
}
